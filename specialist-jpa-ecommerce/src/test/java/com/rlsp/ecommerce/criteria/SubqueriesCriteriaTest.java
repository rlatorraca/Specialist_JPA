...
import com.rlsp.ecommerce.model.ItemPedidoId_;
...
//                " p.total > (select avg(total) from Pedido)";
        ...
        Subquery<BigDecimal> subquery = criteriaQuery.subquery(BigDecimal.class);
        Root<Pedido> subqueryRoot = subquery.from(Pedido.class);
        subquery.select(criteriaBuilder.avg(subqueryRoot.get(Pedido_.total)).as(BigDecimal.class)); //faca o CASTING para BigDecimal pq o AVG retorna um DOUBLE
...
//                " p.preco = (select max(preco) from Produto)";
        ...
        criteriaQuery.where(criteriaBuilder.equal(root.get(Produto_.preco), subquery)); // Compara o root(da query) com a subquery
...
//        Todos os produtos que sempre foram vendidos(e comprados) pelo mesmo preço, mesmo tendo sido mudado o valor de venda
//			** apenas 1 venda com o produto tbem sera retorna, memso que tenha preco difernete na tabela de produtos, pois tem apenas 1 venda existente em Item_Pedido 
...
//        Todos os produtos que já foram vendidos, pelo menos (Qualquer / ANY), uma vez pelo preço atual.
//        String jpql = "select p from Produto p " +
//                " where p.preco = ANY (select precoProduto from ItemPedido where produto = p)";
...
//        Todos os produtos não foram vendidos mais depois que encareceram e foram vendidos pelo menos 1 vez
//        String jpql = "select p from Produto p where " +
